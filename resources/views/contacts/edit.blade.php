...
<body class="bg-slate-50 min-h-screen py-10 px-5 text-slate-900">
    <div class="mx-auto max-w-2xl">
        <header class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-4xl font-semibold tracking-tight">Edit Contact</h1>
                <p class="mt-2 text-sm text-slate-600">Update the selected contact details.</p>
            </div>
        ...
        <form action="{{ route('contacts.update', $contact) }}" method="POST" class="space-y-6 rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
            @csrf
            @method('PUT')
